...
async function getWhatsAppCredentials(
  ...
  console.log(
    "[Broadcast Service] Fetching WhatsApp credentials for userId:",
    userId
  );

  const admin = (await mongodb.Integration.findOne({ userId: userId })) as {
  ...
  const phoneNumberId = admin?.PHONE_NUMBER_ID;
...
export async function sendBroadcast(
  ...
  if (options.isScheduled && options.scheduledTime) {
    ...
    if (!options.senderId) {
      throw new Error("senderId is required for broadcast");
    }
  ...
  const credentials = await getWhatsAppCredentials(options.senderId || "");
  if (!credentials) {
...
export async function processScheduledBroadcasts(): Promise<void> {
  ...
  try {
    ...
    for (const broadcast of duebroadcasts) {
      ...
      const credentials = await getWhatsAppCredentials(broadcast.senderId);
      if (!credentials) {
        console.error(
