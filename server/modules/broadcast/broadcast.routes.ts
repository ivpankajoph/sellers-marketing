  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        ...
        .json({
          error:
            "Failed to parse Excel file. Make sure it is a valid .xlsx, .xls, or .csv file.",
        });
    }
  }
);
...
router.post("/schedules", async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !messageType || !scheduledAt) {
      return res
        .status(400)
        .json({ error: "Name, message type, and scheduled time are required" });
    }
  ...
  try {
    ...
    if (isScheduled) {
      if (!scheduledTime) {
        return res
          .status(400)
          .json({ error: "scheduledTime is required when scheduling" });
      }
...
router.post("/send-single", async (req: Request, res: Response) => {
  try {
    const { phone, name, messageType, templateName, customMessage, agentId } =
      req.body;
...
router.post("/send-to-list/:listId", async (req: Request, res: Response) => {
  try {
    const { messageType, templateName, customMessage, agentId, campaignName } =
      req.body;
