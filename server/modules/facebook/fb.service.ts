...
import { getIntegrationKey } from "server/helper/getIntegrationKey";
...
async function getFacebookCredentials(
  ...
  const integrationCreds = await integrationService.getDecryptedCredentials(
    userId,
    "facebook"
  );
  ...
  const FB_USER_OR_PAGE_TOKEN = await getIntegrationKey(
    userId,
    "SYSTEM_USER_TOKEN_META"
  );
  const FB_PAGE_ID = await getIntegrationKey(userId, "FB_PAGE_ID");
...
async function getPageAccessToken(userId: string): Promise<string> {
  const creds = await getFacebookCredentials(userId);

  if (!creds) {
    throw new Error(
      "Facebook credentials not configured. Please connect Facebook in Settings > Connected Apps."
    );
  }
  ...
  try {
    ...
    console.log(
      "[FB] Token is a User Access Token, fetching Page Access Token..."
    );
    ...
    if (accountsData.data && accountsData.data.length > 0) {
      ...
      if (targetPage && targetPage.access_token) {
        console.log(
          `[FB] Found Page Access Token for page: ${targetPage.name} (${targetPage.id})`
        );
...
export async function syncLeadForms(userId: string): Promise<LeadForm[]> {
  const creds = await getFacebookCredentials(userId);
  if (!creds) {
    throw new Error(
      "Facebook credentials not configured. Please connect Facebook in Settings > Connected Apps."
  ...
  try {
    ...
    for (const fbForm of data.data || []) {
      const existingForm = await findByField<LeadForm>(
        FORMS_COLLECTION,
        "fbFormId",
        fbForm.id
      );
...
export async function getFormByFbId(
  fbFormId: string
): Promise<LeadForm | null> {
  return findByField<LeadForm>(FORMS_COLLECTION, "fbFormId", fbFormId);
}

export async function syncLeadsForForm(userId:string ,formId: string): Promise<Lead[]> {
  const FB_USER_OR_PAGE_TOKEN = await getIntegrationKey(
    userId,
    "SYSTEM_USER_TOKEN_META"
  );
  ...
  if (!FB_USER_OR_PAGE_TOKEN) {
    throw new Error(
      "Facebook credentials not configured. Please set FB_PAGE_ACCESS_TOKEN."
    );
  }
  ...
  if (!form) {
    ...
    if (!form) {
      console.log(
        `[FB Service] Form not found by fbFormId either, checking all forms...`
      );
      ...
      if (allForms.length > 0) {
        allForms.forEach((f) =>
          console.log(
            `[FB Service] Form: id=${f.id}, fbFormId=${f.fbFormId}, name=${f.name}`
          )
        );
      }

      const foundForm = allForms.find(
        (f) => f.id === formId || f.fbFormId === formId
      );
      if (!foundForm) {
        console.log(
          `[FB Service] Form still not found after checking all forms`
        );
  ...
  console.log(
    `[FB Service] Found form: ${form.name} (fbFormId: ${form.fbFormId})`
  );

  if (!form.fbFormId) {
    throw new Error(
      "Form is missing Facebook Form ID. Please re-sync forms from Facebook."
    );
  }
  ...
  try {
    ...
    for (const lead of newLeads) {
      if (lead.phone && !lead.autoReplySent) {
        console.log(
          `[FB Service] Triggering auto-reply for new lead: ${lead.id}`
        );
        ...
        leadAutoReply
          ...
          .then(async (result) => {
            if (result.success) {
              ...
              if (idx !== -1) {
                ...
                console.log(
                  `[FB Service] Auto-reply status saved for lead ${lead.id}`
                );
              }
            }
          })
          .catch((err) => {
            console.error(
              `[FB Service] Auto-reply failed for lead ${lead.id}:`,
              err
            );
          });
        ...
        triggerDripCampaignsForLead(lead).catch((err) => {
          console.error(
            `[FB Service] Drip campaign trigger failed for lead ${lead.id}:`,
            err
          );
        });
      }
...
async function triggerDripCampaignsForLead(lead: Lead): Promise<void> {
  if (!lead.phone) {
    console.log(
      `[FB Service] Lead ${lead.id} has no phone number, skipping drip campaigns`
    );
    return;
  }
  ...
  try {
    ...
    if (!contact) {
      ...
      console.log(
        `[FB Service] Created new contact for Facebook lead: ${contact.phone}`
      );
    }
    ...
    if (result.enrolled.length > 0) {
      console.log(
        `[FB Service] Enrolled lead ${
          lead.id
        } in drip campaigns: ${result.enrolled.join(", ")}`
      );
    }
    if (result.skipped.length > 0) {
      console.log(
        `[FB Service] Skipped campaigns for lead ${
          lead.id
        }: ${result.skipped.join(", ")}`
      );
    }
    ...
    console.error(
      `[FB Service] Error triggering drip campaigns for lead ${lead.id}:`,
      error
    );
    throw error;
  }
