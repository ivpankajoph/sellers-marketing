...
import { Play, Pause, Zap, FileText, Search, Filter, CheckCircle, Clock, AlertCircle } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600">Manage your lead forms and automation templates</p>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl shadow-md p-4 border-l-4 border-amber-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm">Automated</p>
                <p className="text-2xl font-bold text-gray-900">{formsWithTemplate}</p>
              </div>
              ...
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                ...
                {filterStatus !== 'all' && (
                  <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                    Status: {filterStatus}
                    <button onClick={() => setFilterStatus('all')} className="ml-1 hover:text-purple-900">×</button>
                  </span>
                )}
                {filterTemplate !== 'all' && (
                  <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm flex items-center gap-1">
                    Template: {filterTemplate === 'assigned' ? 'With' : 'Without'}
                    <button onClick={() => setFilterTemplate('all')} className="ml-1 hover:text-green-900">×</button>
                  </span>
                )}
                <button
                  onClick={() => {
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
          ...
          {filteredForms.length === 0 && forms.length > 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 text-lg mb-2">No forms match your filters</p>
              <button
                onClick={() => {
                  setSearchQuery('');
                  setFilterStatus('all');
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {form.assigned_template && (
                        <span className="flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1.5 rounded-full text-sm font-medium">
                          <CheckCircle size={14} />
                          Active
                        </span>
                      )}
                  ...
                  {form.assigned_template && (
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <p className="text-xs text-gray-500">
                        Automation active - New leads will receive the selected template automatically
                      </p>
                    </div>
                  )}
                </div>
